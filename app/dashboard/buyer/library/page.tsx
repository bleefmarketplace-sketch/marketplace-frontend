...
import { 
    ...
    Download, Search, GraduationCap, Loader2, ChevronRight 
} from 'lucide-react';
...
export default function DigitalLibraryPage() {
    ...
    useEffect(() => {
        const loadLibrary = async () => {
            try {
                // Fetch orders and filter for items that are digital
                const orders = await fetcher('/api/orders/my-orders');
                const digitalItems = orders.flatMap(order => 
                    order.items.filter((item: any) => item.product?.type === 'digital' || item.typeSnapshot === 'digital')
                );
    ...
    return (
        <div className="max-w-6xl mx-auto py-10 px-4 space-y-8">
            <div>
                ...
                <p className="text-gray-500 font-medium">Access your purchased farming guides and technical courses.</p>
            </div>
            ...
            {library.length === 0 ? (
                <Card className="p-20 text-center border-2 border-dashed rounded-[3rem]">
                    ...
                    <Button onClick={() => router.push('/marketplace')} className="mt-4 bg-emerald-600">Explore Courses</Button>
                </Card>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {library.map((item) => (
                        <Card key={item.id} className="p-0 overflow-hidden group border-none ring-1 ring-gray-100 shadow-sm hover:shadow-xl transition-all rounded-[2.5rem]">
                            ...
                            <div className="p-6">
                                <h4 className="font-bold text-gray-900 line-clamp-1 mb-2">{item.productSnapshotTitle}</h4>
                                <div className="flex items-center justify-between mt-6">
                                    ...
                                    <Button 
                                        size="sm" 
                                        onClick={() => router.push(`/dashboard/library/${item.product.id}`)}
